...
public class TwoGoodItemCell extends LinearLayout
{
    ...
    public void bindData(List<GOODS> listData)
    {
        init();
        ...
        if (listData.size() > 0)
        {
            final GOODS goodOne = listData.get(0);
            if (null != goodOne && null != goodOne.goods_img)
            {
                imageLoader.displayImage(goodOne.goods_img, good_cell_photo_one, EcmobileApp.options);
            }
            ...
            if (listData.size() > 1)
            {
                ...
                final GOODS goodTwo = listData.get(1);
                if (null != goodTwo && null != goodTwo.goods_img)
                {
                    imageLoader.displayImage(goodTwo.goods_img, good_cell_photo_two, EcmobileApp.options);
                }
